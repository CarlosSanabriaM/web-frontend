...
# This Dockerfile uses 2 variables:
#  - PROD (build-time variable): true or false. If true, sets the Angular build configuration to the production target.
#          This variable is set in the build command with --build-arg. Set this value to true when the web_backend is in production.
#  - PORT (environment variable): Port where the frontend will be listening inside the container. Must be > 1024.
#          This variable can be set in the run command with -e.


# To build the image execute:
#  $ docker build . -t web_frontend:latest --build-arg PROD=<true/false>
# If PROD is true, sets the Angular build configuration to the production target.
...
# Define the build-time variable that specifies if the Angular app have to be built in production mode or not.
# By default, is true.
ARG PROD=true
